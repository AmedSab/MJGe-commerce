...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail_subject = 'MJG account activation e-mail'
            ...
            to_email = email
            sender = 'aa7235845@gmail.com'
            send_email = EmailMessage(mail_subject, message, sender, to=[to_email])
            send_email.send()
            # messages.success(request, 'We have sent you a verification email address to activate your account.')
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            current_site = get_current_site(request)
            mail_subject = 'MJG Password Reset'
            ...
            to_email = email
            sender = 'aa7235845@gmail.com'
            send_email = EmailMessage(mail_subject, sender, message, to=[to_email])
            send_email.send()
            ...
            messages.error(request, 'Account with this email adderess does not exist.')
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user = Accout.objects.get(pk=uid)
            user.set_password(password)
            user.save()
            messages.success(request, 'Password was reseted successfully')
            return redirect(login)
        else:
